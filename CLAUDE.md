...
**Weather work status:**
- `lxc-health-api/` is scaffolded as a Hostinger-ready Express/TypeScript API.
...
- `GET /v1/weather/today` accepts `q`, or `lat` + `lon`, and falls back to
  ...
  have location.
- API/provider config lives in `lxc-health-api/src/config/apis.ts` as
...
- A WeatherAPI.com call was manually verified for Dubai on 2026-07-25 and
  ...
  `lxc-health-api/README.md`.
- Deployment bundles are generated by `Executable/macos_healthapi_package.sh`
  into `lxc-health-api/publish/lxc-health-api-YYYY-MM-DD-HHMM.tar`. Do not use
