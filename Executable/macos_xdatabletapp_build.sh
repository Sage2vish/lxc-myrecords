...
#
# PURPOSE     : One-shot build+launch script for MyHealthHub Tablet (Android).
#               Loads
...
# ============================================================================
#
# macos_xdatabletapp_build.sh — build + launch MyHealthHub Tablet (Android / tablet native project)
#
# Usage:
#   ./macos_xdatabletapp_build.sh          # debug build, installed on whatever adb sees
#   ./macos_xdatabletapp_build.sh release  # release build (installs, does not auto-launch)
#   ./macos_xdatabletapp_build.sh release-only  # release build only, no device required
#
...
#   5. Build the APK via Gradle in lxc-myhealthhub-xdatablet (assembleDebug or assembleRelease).
...
XDA_DIR="$REPO_ROOT/lxc-myhealthhub-xdatablet"
APP_ID="com.lxcmyhealthhub.xdatablet"
...
if [ ! -f "$XDA_DIR/gradlew" ]; then
  fail "lxc-myhealthhub-xda folder" \
    ...
    "Expected file not found: $XDA_DIR/gradlew — this script must live in an Executable/ folder that is a sibling of lxc-myhealthhub-xdatablet in the repo."
fi
echo "    OK"
...
echo "==> [5/6] Building $BUILD_TYPE APK (lxc-myhealthhub-xdatablet)"
cd "$XDA_DIR"
...
# This project builds per-ABI split APKs (e.g. MyHealthHubTablet-debug-arm64-v8a.apk),
